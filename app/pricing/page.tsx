  ...
  const handleApplyDiscount = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const discountText = data.discount.discount_type === 'percentage'
          ? `${data.discount.discount_value}% discount applied!`
          : `€${data.discount.discount_value} discount applied!`;
      ...
      setDiscountMessage({ type: 'error', text: err.message || 'Failed to validate discount code' });
    } finally {
    ...
    try {
      ...
      const response = await fetch("/api/stripe/checkout", {
        ...
        body: JSON.stringify({
          ...
          discountCode: !appliedReferral ? appliedDiscount?.code : null // Only send discount if no referral
        }),
      });
                  ...
                  <>
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="Enter discount code (e.g., SAVE-XXXXXX)"
                    ...
                    {appliedReferral ? (
                      ...
                      <>
                        <div className="flex items-baseline gap-2">
                          <span className="text-5xl font-bold text-green-400">
                            €{appliedDiscount.discount_type === 'percentage'
                              ? Math.round(parseInt(pack.price.replace('€', '').replace(',', '')) * (1 - appliedDiscount.discount_value / 100)).toLocaleString()
                              : Math.max(0, parseInt(pack.price.replace('€', '').replace(',', '')) - appliedDiscount.discount_value).toLocaleString()
                            }
                          </span>
                          <span className="text-2xl text-neutral-500 line-through">
                            {pack.price}
                        ...
                        <p className="text-green-400 text-sm mt-1">
                          {appliedDiscount.discount_type === 'percentage'
                            ...
                            : `€${appliedDiscount.discount_value} off`
                          } with code: {appliedDiscount.code}
                        </p>
                      </>
                    ) : (
                      <div className="flex items-baseline gap-2">
