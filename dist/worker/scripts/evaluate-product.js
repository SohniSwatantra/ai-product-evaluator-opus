...
async function uploadToR2(localPath, key) {
    try {
        const fileContent = await (0, promises_1.readFile)(localPath);
...
function buildDemographicDescription(demographics) {
    return `
Target Customer Profile:
- Age: ${demographics.ageRange} years old
- Gender: ${demographics.gender}
- Income: ${demographics.incomeTier === "low"
        ? "Low (<$50k/year)"
...
async function removeLocalFile(maybePath) {
    if (!maybePath)
        return;
    try {
        await (0, promises_1.unlink)(maybePath);
    }
    catch {
        // Ignore – the file may already have been removed.
    }
}
function resolveScreenshotPath(localPath) {
    if (!localPath) {
        return null;
    }
    // If it's an absolute filesystem path and exists, return it
    if (path_1.default.isAbsolute(localPath) && (0, fs_1.existsSync)(localPath)) {
        return localPath;
    }
    ...
    const withPublic = path_1.default.join(process.cwd(), "public", sanitized);
    if ((0, fs_1.existsSync)(withPublic)) {
        return withPublic;
    }
    // Try direct path from cwd
    const direct = path_1.default.join(process.cwd(), sanitized);
...
async function runEvaluation() {
    ...
    const scrapedData = await (0, web_scraper_1.scrapeWebsite)(validatedUrl, {
        timeout: 30000,
        fullPage: true,
        viewportWidth: 1920,
        viewportHeight: 1080,
    ...
    const productInfo = (0, web_scraper_1.extractProductInfo)(scrapedData);
...
`;
    ...
    const evaluationText = evaluationResponse.content[0].type === "text" ? evaluationResponse.content[0].text : "";
    ...
    try {
        ...
        if (userId) {
            try {
                await (0, db_1.deductUserCredits)(userId, 10, `Main evaluation for ${validatedUrl}`);
...
// ---------------------------------------------------------------------------
function buildScrapedDataAppendix(scrapedData, productInfo) {
    if (scrapedData.error) {
        return `
...
async function enrichWithSSRAnalysis(evaluation, productUrl, demographicDescription) {
    try {
...
Write naturally and conversationally. Be specific about WHY you would or wouldn't buy it. Don't use a rating scale - just explain your thoughts and feelings about purchasing this product.`;
        ...
        const textualAnalysis = response.content[0].type === "text" ? response.content[0].text : "";
        if (!textualAnalysis) {
            return;
        }
        const ssrResults = await (0, ssr_calculator_1.calculateSSR)(textualAnalysis);
        const comparison = (0, ssr_calculator_1.compareMethodologies)(evaluation.overallScore, ssrResults.ssrScore);
        ...
        evaluation.purchaseIntentAnchor = (0, ssr_calculator_1.getAnchorFromSSRScore)(ssrResults.ssrScore);
    }
    catch (error) {
...
async function enrichWithSectionRecommendations(evaluation, productUrl, demographicDescription, scrapedData, productInfo) {
    try {
...
async function pushScreenshotsToR2(evaluation, scrapedData) {
    ...
    if (evaluation.websiteSnapshot?.sectionScreenshots) {
        for (const [section, localPath] of Object.entries(evaluation.websiteSnapshot.sectionScreenshots)) {
            ...
            screenshotUploads.push({
                localPath,
                assign: (url) => {
                    evaluation.websiteSnapshot.sectionScreenshots[section] =
                        url;
                },
            });
        }
    ...
    if (evaluation.sectionedRecommendations) {
        for (const recommendation of evaluation.sectionedRecommendations) {
            if (!recommendation.screenshotPath)
                continue;
            screenshotUploads.push({
    ...
    const uploadedFiles = new Map(); // localPath -> R2 URL
    const uploadTasks = [];
    ...
    const uploadResults = await Promise.allSettled(uploadTasks.map(async ({ item, resolvedPath, key, index }) => {
