...
exports.probeAgentReadiness = probeAgentReadiness;
exports.computeMeasuredFactorScores = computeMeasuredFactorScores;
exports.buildMeasuredSignalsSummary = buildMeasuredSignalsSummary;
...
const PROBE_USER_AGENT = "Mozilla/5.0 (compatible; AX-Evaluator/1.0; +https://2031ai.com)";
...
async function checkResource(url, errors) {
...
function parseRobots(text) {
    ...
    const isBlocked = (agent) => {
        const rules = groups[agent.toLowerCase()] ?? groups["*"] ?? [];
        // Blocked from whole site if there is a bare "Disallow: /"
        return rules.some((r) => r === "/");
    };
    const agentRules = AI_AGENT_USER_AGENTS.filter((a) => groups[a.toLowerCase()] !== undefined || groups["*"] !== undefined).map((a) => ({ userAgent: a, disallowed: isBlocked(a) }));
 ...
 */
async function probeAgentReadiness(url, html) {
    const errors = [];
    let origin;
    try {
    ...
    const [llms, llmsFull, wellKnown, agents, robotsRes, sitemap, openapi, wellKnownRoot, mdNeg, htmlNeg,] = await Promise.all([
    ...
    if (robotsRes && robotsRes.ok) {
        ...
        if (robotsText.trim() && !ct.includes("text/html")) {
            ...
            signals.robotsTxt = {
                present: true,
                allowsAiAgents: parsed.agentRules.length === 0 ? true : blockedCount < parsed.agentRules.length,
...
function computeMeasuredFactorScores(s) {
    // Structured Data
    let structured = 20;
    if (s.structuredData.jsonLdBlocks > 0)
        structured += 40;
    if (s.structuredData.schemaTypes.length > 0)
        structured += 20;
    ...
    if (s.llmsTxt.present)
        negotiation += 40;
    if (s.llmsFullTxt.present || s.wellKnownLlmsTxt.present)
        negotiation += 10;
    if (s.contentNegotiation.supportsMarkdown)
        negotiation += 25;
    else if (s.contentNegotiation.supportsPlainText)
        negotiation += 10;
    if (s.agentsMd.present)
        negotiation += 10;
    // API Availability (hard to fully measure; reward discoverable surfaces)
    let api = 20;
    if (s.apiSurface.hasOpenApi)
        api += 50;
    if (s.apiSurface.hasWellKnown)
        api += 15;
    return {
 ...
 */
function buildMeasuredSignalsSummary(s) {
    const yn = (b) => (b ? "YES" : "NO");
